...
@Component({
  selector: 'app-estilista-turno-edit',
  templateUrl: './estilista-turno-edit.component.html',
  styleUrls: ['./estilista-turno-edit.component.css']
})
export class EstilistaTurnoEditComponent implements OnInit {
  Estilista: Estilista[] = []
  sExiste: boolean;
  id!: string;
  ...
  onSave(body: ITurnos) {
    this.turnoService.actualizarTurno(this.id, body).subscribe({
      ...
      next: (res) => {
        ...
        this.route.navigateByUrl('/dashboard/turno/estilista/');
      },
      error: (error) => {
